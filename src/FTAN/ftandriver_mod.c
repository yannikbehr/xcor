...
int main (int argc, char **argv)
{
  ...
  for(j=0;j<counter;j++){
	...
	if(ii==0)
	  {
	    tmin=5;
	    tmax=14;
	    ff = fopen("/home/behrya/dev/auto/src/FTAN/dispcurve_5-14.txt", "r");
	    if(NULL == ff) {
	      fprintf(stderr, "cannot open file ...\n");
	      return EXIT_FAILURE;
	    }
	    i=0;
	    while((fscanf(ff,"%lf  %lf\n",&pred_1[0][i], &pred_1[1][i])) != EOF )
	      i++;
	    npred_1 = i;
	    fclose(ff);
	  }
	else if(ii==1)
	  {
	    tmin=10;
	    tmax=25;
	    ff = fopen("/home/behrya/dev/auto/src/FTAN/dispcurve_10-25.txt", "r");
	    if(NULL == ff) {
	      fprintf(stderr, "cannot open file ...\n");
	      return EXIT_FAILURE;
	    }
	...
	else if(ii==2)
	  {
	    tmin=15;
	    tmax=35;
	    ff = fopen("/home/behrya/dev/auto/src/FTAN/dispcurve_15-35.txt", "r");
	    if(NULL == ff) {
	      fprintf(stderr, "cannot open file ...\n");
	      return EXIT_FAILURE;
	    }
	...
	printf("--> tmin = %lf   tmax = %lf\n",tmin,tmax);
	/* Make prediction based on 7-month stack               */
	/*for(i = 0; i < npred_1; i++) {
	  printf("period is: %lf   velocity is: %lf\n",pred_1[0][i], pred_1[1][i]);
	  }*/
