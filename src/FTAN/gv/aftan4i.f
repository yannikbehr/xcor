...
c perc    - minimal length of of output segment vs freq. range, % 
c           (real*8)
...
c          tamp      -  time to the beginning of ampo table, s 
c                       (real*8)
...
c   write(*,*)'T0= ',2.0d0*pi/om0,', tg0= ',tg0
...
c spectra tapering
      call tapers(omb,ome,dom,alpha,ns,omstart,inds,inde,omdom,ampdom)
...
c normalization amp diagram to 100 Db with three decade cutting
      ...
      do j = 1,ntall
        ...
        enddo
      enddo
c construction reference indices table ind. It points to local 
c maxima.
c table ipar contains three parameter for each local maximum:
c tim - group time; tvis - observed period; ampgr - amplitude 
c values in Db
      ici = 0
      do k = 1,nf
