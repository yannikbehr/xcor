c
c aftan4 function. Provides regular ftan analysis, jumps correction,
c noise RMS computations (SNR + signal halfwidth) and amplitude map 
c output for input periods.
c
c
c Autor: M.Barmibe,CIEI,CU. Date: Feb 08,2006. Version: 1.41
c
      subroutine aftan4(n,sei,t0,dt,delta,vmin,vmax,tmin,tmax,tresh,
     *           ffact,perc,npoints,taperl,nfin,fsnr,nfout1,arr1,nfout2,
     *           arr2,tamp,nrow,ncol,amp,ierr);
c======================================================================
c Parameters for aftan4 function:
c Input parameters:
...
c          arr1(:,6) -  maximum half width, s (real*8)
...
c          arr2(:,6) -  maximum half width, s (real*8)
...
c======================================================================
      ...
      real*8    perc,taperl,tamp,arr1(7,100),arr2(6,100)
      ...
      real*8    pha(32768,100),amp(32768,100),ampo(32768,100),tim(100)
...
c ---
      ...
      real*8    wdth(100),wdth1(100),wdtht(100),snrt(100)
      integer*4 iflag,ierr1,nindx,imax,iimax,ipos,ist,ibe,nfout2
      integer*4 mm,mi,iml,imr,indl,indr,indx(100)
      real*8    lm,rm
...
c tapering both ends of seismogram
      call taper(max0(nb,ntapb+1),min0(ne,n-ntape),n,sei,ntapb,ntape,s,
     *     ns);
...
c
c aftan4i function. Provides ftan analysis with phase match filter,
c jumps correction, noise RMS computations (SNR + signal halfwidth) 
c and amplitude map output for input periods.
c
c
c Autor: M. Barmine,CIEI,CU. Date Feb: 08, 2006. Version: 1.41l
c
      subroutine aftan4i(n,sei,t0,dt,delta,vmin,vmax,tmin,tmax,tresh,
     *           ffact,perc,npoints,taperl,nfin,fsnr,npred,pred,nfout1,
     *           arr1,nfout2,arr2,tamp,nrow,ncol,amp,ierr);
c======================================================================
c Parameters for aftan4i function:
c Input parameters:
c n       - number of input samples, (integer*4)
c sei     - input array length of n, (real*4)
c t0      - time shift of SAC file in seconds, (real*8)
...
c npred   - length of prediction table
c pred    - prediction table: pred[0][] - periods in sec,
...
c ---
      ...
      real*8    om0,tg0,omstart,dw,pha_corr
...
cxx   write(*,*)'T0= ',2.0d0*pi/om0,', tg0= ',tg0
c seismgram tapering
      nb = max0(2,nint(delta/vmax/dt))
      tamp = (nb-1)*dt+t0;
      ne = min0(n,nint(delta/vmin/dt))
...
c velocity for FTAN map
      ...
      ntime = ne-nb+1
c tapering both ends of seismogram
      call taper(max0(nb,ntapb+1),min0(ne,n-ntape),n,sei,
     *     ntapb,ntape,s,ns);
c prepare FTAN filters
      dom = 2*pi/ns/dt
      step =(dlog(omb)-dlog(ome))/(nf -1)
     ...
     *            ampdom)
      ...
      do i = 1,ns
          ...
          elseif(i.gt.inde) then
              ...
              call dsplint(ip+1,om0,omdom(i),pha_corr,ierr)
...
c cutting impulse response in time
      dw = om(1)-om(nf)
      call tgauss(fsnr,tg0,dw,dt,ns,env,spref)

c      do i=1,ns
c         write(*,*)i,cdabs(env(i)),cdabs(spref(i))
c      enddo
...
c get velocity for low integral boundary
      call dspline(ip,npred,x,y,2,0.0d0,2,0.0d0)
      call dsplder(ip,om0,Gt0,s,ss,ierr)
c construct spline for ph filter
      do i = 1,npred
          y(i) = y(i) - Gt0
      enddo
      call dspline(ip+1,npred,x,y,2,0.0d0,2,0.0d0)
      om1 = om0
      return
      end
