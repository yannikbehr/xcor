...
c
     ...
     *           phprper,phprvel,nfout1,arr1,nfout2,arr2,tamp,nrow,
     *           ncol,amp,ierr);
c======================================================================
...
c======================================================================
      ...
      real*8    pha(32768,32),amp(32768,32),ampo(32768,32),tim(100)
...
c ---
      ...
      integer*4 mm,mi,iml,imr,indl,indr,nphpr,indx(100)
...
c evaluation of spline polinomial forms for phase match filter
      ip = 1
      call predcur(ip,delta,dsqrt(omb*ome),npred,pred,om0,tg0)
...
c velocity for FTAN map
      ...
      ntime = ne-nb+1
c tapering both ends of seismogram
      call taper(max0(nb,ntapb+1),min0(ne,n-ntape),n,
     *           sei,ntapb,ntape,s,ns);
c prepare FTAN filters
      dom = 2*pi/ns/dt
      step =(dlog(omb)-dlog(ome))/(nf -1)
c log scaling for frequency
