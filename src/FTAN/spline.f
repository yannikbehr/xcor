...
c-----------------------------------------------------------------------
      entry SPLDER (i0,ik,x,y,m,xt,s,sd,sdd,ierr)
c-----------------------------------------------------------------------
      ierr = 0
      s=0.
      sd=0.
      sdd=0.
      n1=ik-1
      if(fsig(x(i0),x(ik),xt).gt.0.) then
          ierr = 1
          return
      endif
      do 10 j=i0,n1
   ...
   11 h=x(j+1)-x(j)
     ...
     *  +(y(j+1)-y(j))/h
      ...
      return
c-----------------------------------------------------------------------
      entry SPLINT (i0,ik,x,y,m,sa,sb,sint,ierr)
c-----------------------------------------------------------------------
      ierr = 0
      j1 = 1
      j2 = 1
      sint=0.
      if(fsig(x(i0),x(ik),sa).gt.(0.).or.fsig(x(i0),x(ik),sb).gt.0.) then
          ierr = 1
          return
      endif
      n1=ik-1
