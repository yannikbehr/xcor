...
#define STRING 300
#define SSTRING 150
#define INPUT_ARRAY_SIZE  4320000
...
float seis_out[INPUT_ARRAY_SIZE*2];
...
int main (int na, char **arg)
{
	...
	lag        = iniparser_getdouble(dd, "xcor:lag", 3000);
				...
				for( jsta2 = jsta1; jsta2 < sdb.nst; jsta2++ ) {
					if(sdb.rec[ine][jsta2].n > 0){
						...
						else
						{
							...
							offset = (int)( lag/sdb.rec[ine][jsta2].dt + 0.5);
							cor[offset] = seis_out[0];
							for( i = 1; i< (offset+1); i++)
							{
								cor[offset-i] =  seis_out[i];
								cor[offset+i] =  seis_out[ns-i];
							}
							...
							shdamp1.npts =  2*offset+1;
							shdamp1.b    = -(offset)*shdamp1.delta;
							shdamp1.unused1 = 1;
...
void make_dir(int ie, char *cordir, char *pbdir, char *daydir){
	...
	result=strrchr(pbdir,'/');
	dname = strdup(result+1);
	sprintf(bnddir,"%s/%s",cordir,dname);
