...
float *myxcor_fftw(float *sig1, float *sig2, int len, int lag, double *in, 
		   fftw_complex *out, fftw_complex *out2, fftw_plan pf, fftw_plan pb);
...
/*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
int do_cor(int lag , char *cordir, char* pbdir, char *prefix, int len)
{
  int ine, jsta1, jsta2;
  ...
  pb = fftw_plan_dft_c2r_1d(n,out,in,FFTW_ESTIMATE);
  z1 = (float *)malloc(len*sizeof(float));
  z2 = (float *)malloc(len*sizeof(float));
  sigE = (float *)malloc(len*sizeof(float));
  sigN = (float *)malloc(len*sizeof(float));
  if(lag<len){
    cl = lag;
  }else{
    cl = len-1;
  }
  xcor = (float *)malloc(sizeof(float)*(2*cl+1));
...
int write_xcor(float *xcor, int len, int lag, 
	       char *mondir, int ine, int jsta1, int jsta2, char *app){
...
float *myxcor_fftw(float *sig1, float *sig2, float *sigout, int len, int lag, double *in, 
		   fftw_complex *out, fftw_complex *out2, fftw_plan pf, fftw_plan pb){
  int n, i, cl;
  double *x, *y, buff;
  float *sigout;
  ...
  memcpy(in,y,sizeof(double)*n);
  fftw_execute(pf);
  memcpy(out2,out,sizeof(fftw_complex)*((n/2)+1));
  memcpy(in,x,sizeof(double)*n);
  fftw_execute(pf);
  for(i=0;i<((n/2)+1);i++){
    buff = out[i][0];
    out[i][0] = (out[i][0]*out2[i][0]+out[i][1]*out2[i][1])/n;
    out[i][1] = (out[i][1]*out2[i][0]-buff*out2[i][1])/n;
  }
  fftw_execute(pb);
  for(i=0;i<cl;i++){
    sigout[i]=(float)in[n-cl+i];
    sigout[cl+1+i]=(float)in[i+1];
  }
  sigout[cl]=(float)in[0];
  free(x);free(y);
  return sigout;
}
